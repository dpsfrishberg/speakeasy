# -*- coding: utf-8 -*-
from south.utils import datetime_utils as datetime
from south.db import db
from south.v2 import SchemaMigration
from django.db import models


class Migration(SchemaMigration):

    def forwards(self, orm):
        # Adding model 'Article'
        db.create_table(u'speakeasy_core_article', (
            (u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('slug', self.gf('django.db.models.fields.CharField')(max_length=200)),
        ))
        db.send_create_signal(u'speakeasy_core', ['Article'])

        # Adding model 'Node'
        db.create_table(u'speakeasy_core_node', (
            (u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('content', self.gf('django.db.models.fields.TextField')()),
            ('article', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['speakeasy_core.Article'])),
        ))
        db.send_create_signal(u'speakeasy_core', ['Node'])

        # Adding model 'ArticleNode'
        db.create_table(u'speakeasy_core_articlenode', (
            (u'node_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['speakeasy_core.Node'], unique=True, primary_key=True)),
        ))
        db.send_create_signal(u'speakeasy_core', ['ArticleNode'])

        # Adding model 'SpeakeasyComment'
        db.create_table(u'speakeasy_core_speakeasycomment', (
            (u'id', self.gf('django.db.models.fields.AutoField')(primary_key=True)),
            ('node', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['speakeasy_core.Node'])),
            ('user', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['auth.User'])),
            ('article', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['speakeasy_core.Article'])),
        ))
        db.send_create_signal(u'speakeasy_core', ['SpeakeasyComment'])

        # Adding model 'CommentNode'
        db.create_table(u'speakeasy_core_commentnode', (
            (u'node_ptr', self.gf('django.db.models.fields.related.OneToOneField')(to=orm['speakeasy_core.Node'], unique=True, primary_key=True)),
            ('comment', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['speakeasy_core.SpeakeasyComment'])),
        ))
        db.send_create_signal(u'speakeasy_core', ['CommentNode'])


    def backwards(self, orm):
        # Deleting model 'Article'
        db.delete_table(u'speakeasy_core_article')

        # Deleting model 'Node'
        db.delete_table(u'speakeasy_core_node')

        # Deleting model 'ArticleNode'
        db.delete_table(u'speakeasy_core_articlenode')

        # Deleting model 'SpeakeasyComment'
        db.delete_table(u'speakeasy_core_speakeasycomment')

        # Deleting model 'CommentNode'
        db.delete_table(u'speakeasy_core_commentnode')


    models = {
        u'auth.group': {
            'Meta': {'object_name': 'Group'},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '80'}),
            'permissions': ('django.db.models.fields.related.ManyToManyField', [], {'to': u"orm['auth.Permission']", 'symmetrical': 'False', 'blank': 'True'})
        },
        u'auth.permission': {
            'Meta': {'ordering': "(u'content_type__app_label', u'content_type__model', u'codename')", 'unique_together': "((u'content_type', u'codename'),)", 'object_name': 'Permission'},
            'codename': ('django.db.models.fields.CharField', [], {'max_length': '100'}),
            'content_type': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['contenttypes.ContentType']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '50'})
        },
        u'auth.user': {
            'Meta': {'object_name': 'User'},
            'date_joined': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'email': ('django.db.models.fields.EmailField', [], {'max_length': '75', 'blank': 'True'}),
            'first_name': ('django.db.models.fields.CharField', [], {'max_length': '30', 'blank': 'True'}),
            'groups': ('django.db.models.fields.related.ManyToManyField', [], {'to': u"orm['auth.Group']", 'symmetrical': 'False', 'blank': 'True'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'is_active': ('django.db.models.fields.BooleanField', [], {'default': 'True'}),
            'is_staff': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            'is_superuser': ('django.db.models.fields.BooleanField', [], {'default': 'False'}),
            'last_login': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime.now'}),
            'last_name': ('django.db.models.fields.CharField', [], {'max_length': '30', 'blank': 'True'}),
            'password': ('django.db.models.fields.CharField', [], {'max_length': '128'}),
            'user_permissions': ('django.db.models.fields.related.ManyToManyField', [], {'to': u"orm['auth.Permission']", 'symmetrical': 'False', 'blank': 'True'}),
            'username': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '30'})
        },
        u'contenttypes.contenttype': {
            'Meta': {'ordering': "('name',)", 'unique_together': "(('app_label', 'model'),)", 'object_name': 'ContentType', 'db_table': "'django_content_type'"},
            'app_label': ('django.db.models.fields.CharField', [], {'max_length': '100'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'model': ('django.db.models.fields.CharField', [], {'max_length': '100'}),
            'name': ('django.db.models.fields.CharField', [], {'max_length': '100'})
        },
        u'speakeasy_core.article': {
            'Meta': {'object_name': 'Article'},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'slug': ('django.db.models.fields.CharField', [], {'max_length': '200'})
        },
        u'speakeasy_core.articlenode': {
            'Meta': {'object_name': 'ArticleNode', '_ormbases': [u'speakeasy_core.Node']},
            u'node_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': u"orm['speakeasy_core.Node']", 'unique': 'True', 'primary_key': 'True'})
        },
        u'speakeasy_core.commentnode': {
            'Meta': {'object_name': 'CommentNode', '_ormbases': [u'speakeasy_core.Node']},
            'comment': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['speakeasy_core.SpeakeasyComment']"}),
            u'node_ptr': ('django.db.models.fields.related.OneToOneField', [], {'to': u"orm['speakeasy_core.Node']", 'unique': 'True', 'primary_key': 'True'})
        },
        u'speakeasy_core.node': {
            'Meta': {'object_name': 'Node'},
            'article': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['speakeasy_core.Article']"}),
            'content': ('django.db.models.fields.TextField', [], {}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'})
        },
        u'speakeasy_core.speakeasycomment': {
            'Meta': {'object_name': 'SpeakeasyComment'},
            'article': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['speakeasy_core.Article']"}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'node': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['speakeasy_core.Node']"}),
            'user': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['auth.User']"})
        }
    }

    complete_apps = ['speakeasy_core']